...
### Read (for analysis)
Reading to **edit** -> Read correct for exact edit context. Reading to **analyze/explore/summarize** -> `mcp__context-mode__ctx_read(path, mode: "map" | "outline" | "symbols")`, then `mcp__context-mode__ctx_read(path, mode: "slice", start, end)` for needed ranges. Use `mode: "full"` only with a reason. If `mcp__context-mode__ctx_read` is unavailable, fall back to `mcp__context-mode__ctx_execute_file(path, language, code)`.
