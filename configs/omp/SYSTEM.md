...
### read (for analysis)
Reading to **edit** -> read correct for exact edit context. Reading to **analyze/explore/summarize** -> `ctx_read(path, mode: "map" | "outline" | "symbols")`, then `ctx_read(path, mode: "slice", start, end)` for needed ranges. Use `mode: "full"` only with a reason. If `ctx_read` is unavailable, fall back to `ctx_execute_file(path, language, code)`.
