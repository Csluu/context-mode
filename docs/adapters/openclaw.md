...
The adapter includes a workspace router (`src/adapters/openclaw/workspace-router.ts`) that resolves project paths from OpenClaw event metadata and environment hints, ensuring session databases and routing instructions are scoped per-workspace.
