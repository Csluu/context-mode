 ...
 *
 * Default is now `0` (disabled) — see issue #592. The 15-min default
 * stranded Claude Code + Codex + Cursor adapters which do not auto-respawn
 * the MCP child after clean exit, leaving all ctx_* tools surfaced as
 * "MCP server disconnected" until the user manually reloads. Hosts that
 * actually leak (OpenCode, KiloCode) opt back in by setting a non-zero
 * `idleTimeoutMs` explicitly via `startLifecycleGuard({ idleTimeoutMs })`
 * at bootstrap, or by exporting `CONTEXT_MODE_IDLE_TIMEOUT_MS` in their
 * launcher env.
 *
