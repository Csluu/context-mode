...
const OPENCLAW_BRIDGE_TOOL_DEFS: readonly OpenClawToolDef[] = [
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        compact: { type: "boolean", description: "Terser map/outline/symbols/slice output with stable line numbers and reduced headers" },
  ...
  },
  {
    name: "ctx_code",
    description:
      "Static code navigation without LSP or daemon: symbol search, symbol reads, file outlines, refs-lite, related files, likely tests, and bounded context packs.",
    parameters: {
      type: "object",
      properties: {
        action: { type: "string", description: "index | file_outline | find_symbol | read_symbol | refs_light | related_files | likely_tests | pack" },
        projectDir: { type: "string", description: "Optional project root override" },
        query: { type: "string", description: "Symbol query for find_symbol/read_symbol/pack" },
        symbol: { type: "string", description: "Symbol name alias for query" },
        file: { type: "string", description: "Optional project-relative or absolute file path" },
        kind: { type: "string", description: "Optional symbol kind filter such as function, class, interface, struct, method" },
        limit: { type: "number", description: "Max returned matches" },
        budgetBytes: { type: "number", description: "Hard output byte budget" },
        includeImports: { type: "boolean", description: "Include compact import context when reading a symbol" },
        compact: { type: "boolean", description: "Return terse output" },
        json: { type: "boolean", description: "Return JSON" },
      },
      additionalProperties: true,
    },
    execute: cliRedirect("ctx_code"),
  },
  {
    name: "ctx_index",
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        explain: { type: "boolean", description: "Return pretty JSON explanation. Tiny direct pass-through commands may return compact JSON." },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        projectDir: { type: "string", description: "Optional project root override" },
        semantic: { type: "boolean", description: "Attempt provider-backed semantic diff" },
        summary: { type: "boolean", description: "Return compact summary" },
        risk: { type: "boolean", description: "Return risk summary" },
        rawSidecar: { type: "boolean", description: "Store redacted raw diff sidecar" },
        staged: { type: "boolean", description: "Inspect staged changes" },
        from: { type: "string", description: "Optional base Git revision" },
        to: { type: "string", description: "Optional target Git revision" },
        json: { type: "boolean", description: "Return machine-readable JSON" },
        maxInputBytes: { type: "number", description: "Max Git output bytes" },
      },
      additionalProperties: true,
    },
