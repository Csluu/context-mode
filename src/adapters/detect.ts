...
export function detectPlatform(clientInfo?: { name: string; version?: string }): DetectionSignal {
  ...
  // ── HIGHEST priority: explicit host override (recommended) ─────────────
  // Set by the platform's MCP manifest at spawn time. Eliminates every
  // detection-heuristic bug class — vscode-copilot vs claude-code (#539),
  // env-leak ambiguity (#545), foreign identification (#561), consumer-set
  // workspace vars (#542). Plugins SHOULD set this in their mcpServers env
  // block. `CONTEXT_MODE_HOST` is the canonical name; `CONTEXT_MODE_PLATFORM`
  // is kept as a backward-compat alias.
  const hostOverride =
    process.env.CONTEXT_MODE_HOST ?? process.env.CONTEXT_MODE_PLATFORM;
  if (hostOverride && validPlatforms.includes(hostOverride as PlatformId)) {
    return {
      platform: hostOverride as PlatformId,
      confidence: "high",
      reason: process.env.CONTEXT_MODE_HOST !== undefined
        ? `CONTEXT_MODE_HOST=${hostOverride} explicit override`
        : `CONTEXT_MODE_PLATFORM=${hostOverride} (legacy alias)`,
    };
  }

  // ── Second priority: MCP clientInfo (from initialize handshake) ────────
  // Reliable when the immediate parent is a real MCP host. Less reliable
  // when relayed through Pi's bridge or sub-agent MCP wrappers, which is
  // why CONTEXT_MODE_HOST wins above.
  if (clientInfo?.name) {
